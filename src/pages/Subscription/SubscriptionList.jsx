  ...
  const SubscriptionDelete = async (id) => {
    ...
    try {
      ...
      if (deleteSub.message == "SUBSCRIPTION DELETED!") {
        ...
        setSub((prev) => prev.filter((item) => item.id !== id));
      } else {
        Swal.fire({
          icon: "error",
          title: "error",
    ...
    <>
      <div className="subscription-container">
        <div className="subscriptionData-container">
          <h1>SUBSCRIPTION!</h1>
          <button className="add-btn" onClick={() => navigate("/subCreate")}>
            +ADD
          </button>
          <table>
            <thead>
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <button
                        ...
                        onClick={() => SubscriptionDelete(item.id)}
                      >
                        <Trash2 size={18} />
                      </button>
                      <button
                        className="btn-view"
                        onClick={() =>
                          navigate(`/Subscription/View/${item.id}`)
                        }
                      >
                        <Eye size={18} />
                      </button>
                      <button
                        className="btn-edit"
                        onClick={() =>
                          navigate(`/Subscription/Edit/${item.id}`)
                        }
                      >
                        <PencilIcon size={18} />
                      </button>
