  ...
  {booking.length === 0 ? (
    ...
    booking.map((item, index) => (
      <tr key={index}>
        ...
        <td>{item.comment}</td>
        <td>
          <button className="btn-delete"  onClick={() => DeleteBooking(item.id)}>
            <Trash2 size={18} />
          </button>
          <button className="btn-view"  onClick={()=>navigate(`/booking/View/${item.id}`)}>
            <Eye size={18} />
             
          </button>
           <button className="addbooking-btn" onClick={()=>navigate(`/booking/Update/${item.id}`)}>
                      <PencilIcon size={18} />
                    </button>
        </td>
      </tr>
